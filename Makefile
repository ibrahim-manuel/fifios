...
C_SOURCES = $(wildcard kernel/arch/i386/*.c kernel/arch/i386/drivers/keyboard/*.c kernel/kernel/*.c libc/stdio/*.c libc/stdlib/*.c libc/string/*.c)

# Geração das listas de objetos
ASM_OBJECTS = $(patsubst kernel/arch/i386/%.asm, output/kernel/arch/i386/%.o, $(ASM_SOURCES))
...
output/kernel/arch/i386/%.o: kernel/arch/i386/%.asm
	@mkdir -p $(dir $@)
	$(ASM) -f elf32 -o $@ $<
